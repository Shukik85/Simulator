...
DEFAULT_MECHANICS_CONFIG = ExcavatorMechanicsConfig(
    ...
    # 2× ГЦ 110.80х1000 → один эквивалентный
    boom_cyl=CylinderGeometry(
    ...
    # ГЦ 100.63х900 (соединяет A на рукояти с P на рычаге)
    bucket_cyl=CylinderGeometry(
    ...
    bucket_lever=BucketLeverMechanismParams(
        lever_length_m=0.52,
        rod_length_m=0.480,
        anchor_A=(-0.135, 0.51),       # ГЦ ковша на рукояти
        pivot_C=(1.685, 0.0),          # ось рычага
        pivot_D=(2.0, 0.0),            # ось ковша
        E_local=(-0.056503, 0.371227), # крепление тяги на ковше
        bucket_tip_local=(1.35, 0.0),  # режущая кромка
    ),
)
