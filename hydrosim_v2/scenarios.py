...
class ScenarioGenerator:
    ...
    def sample_profile(self, mode: str) -> ScenarioProfile:
        payload = float(self.rng.uniform(0.0, 2000.0))
        aggress = float(self.rng.uniform(0.35, 0.95))
        soil_factor = float(1.0 + self.rng.uniform(-0.25, 0.25))

        duration = 60.0

        if mode == "digging_light":
            return ScenarioProfile(mode, duration, 0.25 * payload, 0.8 * soil_factor, aggress, "Light digging")
        if mode == "digging_medium":
            return ScenarioProfile(mode, duration, 0.6 * payload, 1.0 * soil_factor, aggress, "Medium digging")
        if mode == "digging_heavy":
            return ScenarioProfile(mode, duration, 0.9 * payload, 1.3 * soil_factor, aggress, "Heavy digging")
        if mode == "swing_only":
            return ScenarioProfile(mode, duration, 0.3 * payload, 0.2 * soil_factor, aggress, "Swing only")
        if mode == "boom_up":
            return ScenarioProfile(mode, duration, 0.5 * payload, 0.2 * soil_factor, aggress, "Boom up")
        if mode == "boom_down":
            return ScenarioProfile(mode, duration, 0.5 * payload, 0.2 * soil_factor, aggress, "Boom down")
        if mode == "combined":
            return ScenarioProfile(mode, duration, payload, 1.1 * soil_factor, aggress, "Combined cycle")
        return ScenarioProfile("idle", duration, 0.0, 0.0, 0.0, "Idle")
    ...
    def command(self, prof: ScenarioProfile, t: float) -> Dict[str, float]:
        ...
        u = {"boom": 0.0, "arm": 0.0, "bucket": 0.0, "pumpspeed": 1800.0}
        ...
        for k in ("boom", "arm", "bucket"):
            u[k] = float(np.clip(u[k], -1.0, 1.0))
        u["pumpspeed"] = float(np.clip(u["pumpspeed"], 800.0, 2500.0))
        return u
